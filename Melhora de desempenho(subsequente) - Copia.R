...
for (i in 1:length(nomesLog)){
  if ( (str_detect(nomesLog[i], "ADM") != TRUE) && (str_detect(nomesLog[i], "CAP") != TRUE) && 
       ...
       (str_detect(nomesLog[i], "SUAP") != TRUE)&&
       ...
       (str_detect(nomesLog[i], "-CRC-") != TRUE)&& (str_detect(nomesLog[i], "Debora Santos Maciel") != TRUE)&&
       (str_detect(nomesLog[i], "-INET-") != TRUE)&& (str_detect(nomesLog[i], "Manoel Amaral de Souza Neto") != TRUE)&&
       (str_detect(nomesLog[i], "CINET") != TRUE)&& (nomesLog[i] != "Gustavo Vieira da Silva - UBA-SMSI-T4")&&
       ...
       (str_detect(nomesLog[i], "Adriana Silva Santos") != TRUE)&& (str_detect(nomesLog[i], "Omar de Jesus Monteiro Filho") != TRUE)&& 
       (nomesLog[i] != "Jamilla Araujo Baltar - SAJ-SINET-T5")&& (nomesLog[i] != "Taineis Araujo de Lima - SAJ-SINET-TA")&&
       (nomesLog[i] != "Selma Santana Nunes - SAJ-SINET-T5")&& (nomesLog[i] != "Luzimara Souza da Silva - SAJ-SINET-T5")&&
       (nomesLog[i] != "Alessandro Sampaio da Silva - SAJ-SINET-T5")&& (nomesLog[i] != "Jakeline Costa Araujo - SAJ-SINET-T5")&&
       (nomesLog[i] != "Luan de Oliveira - SAJ-SINET-T5")&& (nomesLog[i] != "Gilmaria Teles do Nascimento - SAJ-SINET-T5")&&
       (nomesLog[i] != "David Henrique Mangabeira Jose - SAJ-SINET-T5")&& (nomesLog[i] != "Veronice de Jesus - SAJ-SINET-T5")&&
       (nomesLog[i] != "Derlei Alves dos Santos - SAJ-SINET-T5")&& (nomesLog[i] != "Veronice de Jesus - SAJ-SINET-T5")&&
       (str_detect(nomesLog[i], "SAJ-SMSI-TA") != TRUE)&& (nomesLog[i] != "Geovanne Moreira Cazaes - SAJ-SINET-T5")&&
       (nomesLog[i] != "Antonio Carlos Costa Ribeiro Filho - SAJ-SRC-T3")
  ) {
...
for (i in 1:length(alunosLog)){
  ...
  if((str_detect(alunosLog[i], "CMSI") == TRUE) || (str_detect(alunosLog[i], "CINET") == TRUE)|| (str_detect(alunosLog[i], "SAJ-SINET-TB") == TRUE)
     || (str_detect(alunosLog[i], "UBA-SINF-T1") == TRUE)){
...
setwd("C:\\Users\\IFBA\\Desktop\\subsequente\\3 semestre\\Tec. Redes")
write.csv(dadosMineracao, file = "dadosMineracao Redes(18-11-22).csv")
